...
import com.game.find.word.SentenceCompletion.repository.SentenceCompletionRepository; // Repository'nizin adını kontrol edin
...
@Slf4j
public class SentenceCompletionLoadService {
...
/*
    ...
    public void initCompletionData() {
        try {
            ...
            List<SentenceCompletion> completionList = objectMapper.readValue(inputStream, new TypeReference<List<SentenceCompletion>>() {});
...
*/
    private void bulkSaveInChunks(List<SentenceCompletion> list, int chunkSize) {
        for (int i = 0; i < list.size(); i += chunkSize) {
            ...
            List<SentenceCompletion> chunk = list.subList(i, end);
    ...
    private void executeBulkInsert(List<SentenceCompletion> chunk) {
        BulkOperations bulkOps = mongoTemplate.bulkOps(BulkOperations.BulkMode.UNORDERED, SentenceCompletion.class);

        for (SentenceCompletion sc : chunk) {
